...
@Composable
fun AssessmentResultScreen(navHostController: NavHostController) {
    ...
    AuroscholarAppTheme {
        QuizResultData(context, navHostController, languageData)
    }
}
...
fun QuizResultData(
    ...
    /* var pageName by remember { mutableStateOf("") }
     ...
     if (viewModel.getPracticePage() != "") {
         pageName = viewModel.getPracticePage().toString()
     }*/
//    val languageListName = stringResource(id = R.string.key_lang_list)
//    var languageData :HashMap<String, String>
//    languageData = loginViewModel.getLanguageTranslationData(languageListName)
                ...
                Button(
                    onClick = { /* Handle click */
                        if (isPractice) {
                            ...
                            navHostController.navigate(AppRoute.PracticeConceptList(""))
                            navController.popBackStack()
//                            viewModel.setPracticePage("")
                        }/* else if (loginViewModel.getStudentQuizResultData().conceptText == "Practice 1"
                            || loginViewModel.getStudentQuizResultData().conceptText == "Practice 2"
                            || loginViewModel.getStudentQuizResultData().conceptText == "Practice 3"
                        ) {
                            navHostController.navigate(AppRoute.PracticeConceptList(""))
                            navController.popBackStack()
                        }*/ else {
                    ...
                    Text(
                        text = if (isPractice) {
                            ...
                            loginViewModel.getStudentQuizResultData().conceptText
                        },
                        modifier = Modifier.padding(vertical = 5.dp),
                        color = PrimaryBlue,
                        fontSize = 16.sp,
...
//                navController.navigate(AppRoute.StudentQuizzes.route)
                if (isPractice) {
                    ...
                    navHostController.navigate(AppRoute.StudentPractice.route)
                    navController.popBackStack()
//                            viewModel.setPracticePage("")
                } else {
...
fun QuizResultScreenPreview() {
    AuroscholarAppTheme {
        ...
        var languageData: HashMap<String, String>
        languageData = loginViewModel.getLanguageTranslationData(languageListName)
//        AssessmentResultScreen(navHostController = rememberNavController())
        QuizResultData(context, navHostController, languageData)
    }
}
